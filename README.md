...
**問題**：程式會候選25再精選12回傳檢索結果，那如果想要無損輸入整頁內文，甚至某範圍內整頁內文，要修改程式嗎? 還是只要改變llm問題即可?

**回答**：不需要改程式，只要在提問時用對語法就能觸發無損全頁輸入。

**說明**：現有機制已經支援路徑 B 的「頁碼強制補撈」完全不受 top-25 / top-12 限制。當問題中包含頁碼語法時，系統會用 ChromaDB `where` 條件把該頁所有區塊全部撈出，然後和語意搜尋的結果合併後一起送進 LLM，而不是二選一。
...
**指定頁碼 + 追問**：
    ...
    根據第 10～12 頁，整理所有數據表格

系統會自動用 regex 偵測到「第 N 頁到第 M 頁」或「第 N～M 頁」的模式，啟動補撈。

**一個潛在限制要注意**

強制補撈的所有區塊會和語意搜尋結果**合併後一起進 Cross-Encoder 重排**，最終仍然只輸出前 12 個。如果指定的頁範圍很大（例如第 1 到第 30 頁），補撈出的區塊非常多，而 top-12 的上限會把大部分頁面的內容擠掉。

這種情況下才需要改程式，把 `retriever.py` 裡的 `n_results=12` 調大，或是讓頁碼補撈的區塊**跳過 Cross-Encoder**、直接全部保留進 context。不過要注意 LLM 的 context window 長度是否撐得住。